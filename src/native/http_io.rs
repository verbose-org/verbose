...
/// this transport. Label zero is the enclosing service's close/reset boundary.
struct Asm<'a> {
    code: &'a mut Vec<u8>,
...
impl<'a> Asm<'a> {
    ...
    fn jump(&mut self, cond: Option<u8>, l: usize) {
        if let Some(c) = cond {
            self.bytes(&[0x0f, c]);
        } else {
            self.bytes(&[0xe9]);
    ...
    fn finish(self) -> Vec<usize> {
        ...
        for (at, label) in self.jumps {
            if label == 0 {
                ...
                let target = self.labels[label].expect("unbound HTTP label");
                let rel = i32::try_from(target as i64 - at as i64 - 4).expect("HTTP rel32");
    ...
    fn imm(&mut self, dst: u8, value: i32) {
        self.rex(0, dst);
        self.bytes(&[0xc7, 0xc0 | (dst & 7)]);
        self.i32(value);
    }
    fn op(&mut self, group: u8, dst: u8, value: i32) {
        self.rex(0, dst);
        self.bytes(&[0x81, 0xc0 | (group << 3) | (dst & 7)]);
        self.i32(value);
    }
    fn cmp(&mut self, r: u8, v: i32) {
        self.op(7, r, v);
    }
    fn load(&mut self, r: u8, offset: i32) {
        self.rex(r, BP);
        self.bytes(&[0x8b, 0x85 | ((r & 7) << 3)]);
        self.i32(offset);
    }
    fn store(&mut self, offset: i32, r: u8) {
        self.rex(r, BP);
        self.bytes(&[0x89, 0x85 | ((r & 7) << 3)]);
        self.i32(offset);
    }
    fn lea(&mut self, r: u8, offset: i32) {
        self.rex(r, BP);
        self.bytes(&[0x8d, 0x85 | ((r & 7) << 3)]);
        self.i32(offset);
    }
    fn rip(&mut self, r: u8, target: usize) {
        self.rex(r, 0);
