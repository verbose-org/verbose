...
| runtime input bounds (`number [min, max]`, `text [..N]`) | ENFORCED at field-load — out-of-range input `sys_exit(1)`s before the rule body runs (PR #35 for numbers, PR #146 for text; the text half was an argv-controlled stack overflow) | **STILL not emitted** — the one member of the fail-closed family PR #148 did NOT close. gen0's trampolines load the field and run. Measured: `examples/aes_sbox.verbose` (`b : number [0, 255]`) given 256 / 300 / −5 returns a value at rc 0 where verbosec's binary exits 1. Deferred deliberately, not overlooked: gen0 does not RETAIN the bound. `skip_field_range` *discards* it (a faithful skip, by design since 2026-07-06) and `FCons` carries only `(name_start, name_len, ty, ty_start, ty_len, rest)` — so closing this needs the declared range threaded into the AST across ~77 `FCons` sites, two grammars (`[min, max]` signed pair vs `[..N]`), and emit + size mirrors in BOTH `x86_marshal_fields` and `x86_stdin_fields`. There IS a cheaper road worth taking first: `entry_uses_stdin` already recovers a text bound WITHOUT storing it, by re-scanning the source at `fl_text_ty_end` (`field_text_bound`) — a numeric sibling of that helper would avoid widening `FCons` entirely. Its own slice either way; a half-built version covering some fields or some channels is worse than none, because it implies a guarantee that is not there |
