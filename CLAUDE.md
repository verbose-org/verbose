...
| argv `text` input fields | the field's bytes reach the rule | **reads them as EMPTY** — this one is not a verification gap but a CODEGEN gap, and it is the worst kind: the binary compiles, runs, and returns a wrong number. Probed with no bitwise involved: `byte_at(h.data, i) * 1000 + length(h.data)` over a 128-char argv string returns 0 from gen0 at every index, where verbosec returns 56128 / 48128. Surfaced (not caused) by the bitwise arc, which made 7 text-reading crypto files compile for the first time: `sha512_fold` now returns digest byte 0 = 106 where verbosec and Python `hashlib` both return 207. Affects `sha256_fold`, `sha512_fold`, `ghash_nblocks`, `ladder_recursive`, `p256_ninv_rec`, `p256_scalarmult`, `ed_scalarmult`; the four newly-accepted files with no text field (`bit_ops`, `sha256_prims`, `sha256_round`, `x25519_rec`) are correct. Needs its own slice |
