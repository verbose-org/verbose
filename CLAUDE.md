...
Reachability is confirmed at the byte level, not inferred: for each of the five the kernel-reported trap `ip` is exactly `int3_site + 1` (x86 reports RIP after the 1-byte trap), so the `int3` was *executed*. The common denominator is that gen0's number trampoline is the only finished one — `purchase`'s Ok arm is a Number and works; every arm that must yield TEXT either traps at the placeholder or, where the packed span survives to the printer, emits the span as a decimal integer. **The silent-wrong-answer row is the dangerous one**, and it is the same class as the argv-text and defensive-bounds defects above: a compiler that returns a number that is simply not the answer. Closing this is a real emit slice (a text trampoline for `Result`/record/`output: text`), deliberately not attempted here. Separately, 12 exit 1 on this input set (`allowlist`, `cv_screening`, `ed_scalarmult`, `field_codec`, `ghash_nblocks`, `health_check`, `loan_decision`, `p256_ninv_rec`, `p256_scalarmult`, `read_config`, `sha256_fold`, `sha512_fold`) — those look like legitimate fail-closed paths (`read_config` / `allowlist` open a resource that is absent, `health_check` dials an upstream, several crypto entries hit a declared bound now that PR #149 enforces them), but they were **not** differentially checked one by one, so treat that as unverified rather than clean.
