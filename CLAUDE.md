...
- **VariantConstruct / MatchVariant on a `concept_group` type** — Phase B slice 4a.1 ships the prologue infrastructure (arena reservation, `r11 = arena_base`, node_count_slot, per-group abort label) but no new emit semantics. VariantConstruct of a group concept is refused with a B.4a.2 breadcrumb; MatchVariant on a group concept (scrutinee resolves to `Type::Named(N)` with N inside a group) is refused with a B.4a.3 breadcrumb. Both refusals walk every rule in the program (not just the entry) so an entry rule that delegates to a group-handling callee fails at compile time with the right slice name. Use `--run` (interpreter — B.3) until 4a.2 / 4a.3 land.
