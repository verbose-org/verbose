...
- **WASM (small, growing)**: same principles as native but on its own track. WASM today handles scalar rules: arithmetic, comparisons, logic, `if/then/else`, number `let`, `Call(callee, [Ident(input)])` (inlined), the binary scalar primitives `abs(x)` / `min(a, b)` / `max(a, b)` (slice W1, branchless via `select`), and the foundations of the text type (slice W3a): `Type::Text` as input field (encoded as `(i32 ptr, i32 len)` params into the module's exported linear memory) and as output (multi-value `(i32, i32)` return), plus `Expr::Text` literals embedded in the data section starting at offset 1024 (the first 1 KiB stays reserved for future runtime state — bump allocator metadata in W3b). The (ptr, len) ABI mirrors native's BoundText slot convention exactly, so an auditor reads either backend with the same mental model; the bound travels alongside the data and is never derived from a sentinel. Still refused: text in let bindings (W3b), `concat` and the text primitives (`parse_int`, `length`, `contains`, `starts_with`, `ends_with`) — those land on top of W3a once a bump allocator backs concat outputs. Also still refused: `Result` / `Record` / `collection` outputs, `match_result`, `map`/`filter`/`fold`, quantifiers, `read` / `fetch` / reactions / services. Effects (reactions, services, fetch, read, fork) don't port directly — WASM has no syscalls; bringing them in needs a separate WASI vs host-imports design call. The remaining computational features port mechanically — the AST supports them, the emitter just hasn't been written. This asymmetry is known and deliberate: the security-sensitive target is the native ELF, and WASM's growth follows native once we have a stable convention to mirror.
