...
- **Text-returning rule calls inside `concat(...)` arguments** — Phase 2G unlocked `Call` as a top-level text expression in `emit_text_write_to_fd` (body of `output: text`, Record field value, match_result Err body, Result(text,_) arms). What stays refused is `concat("prefix", helper(p), "suffix")` — that would need a `ConcatArgKind::CallText` variant with runtime sizing (evaluate the call, store (ptr, len) transiently, then treat as BoundText). Separate small phase when needed.
- **Text-returning rule calls in `append_file` content** — reaction emitter dispatches Text / Concat only; Call falls through to the error. Mirror of the concat-args case.
