...
- Text byte count: `length(<text>)` — Number, byte count of a text expression. Counts bytes (not characters): for ASCII this is the obvious answer; for multibyte UTF-8 the result is storage size, not visual length. Native dispatches: text input field → inline `emit_strlen` scan; BoundText (read / fetch / Phase-2I let) → load `len_slot` directly (zero scan because the prologue already counted the bytes); literal → optimizer folds to `Number` at compile time; `concat(<text-only args>)` → 8-byte stack accumulator, each arg's length recurses into the same dispatch and is added in (zero materialisation — bytes are never written, only counted); `Call(<callee>, [Ident(input)])` returning text → Phase 2G-shape inline (same input ident, no callee lets), recurses on `callee.logic.value`. Number-typed args in `concat` (literal numbers, number fields, `now_unix()`, `parse_int`, `abs`, arithmetic) are refused with a clear message that names the offending arg — they'd need an itoa-width helper which is its own slice. JsonEscape / ParseInt as length-arg refused
