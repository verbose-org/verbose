...
| runtime input bounds (`number [min, max]`, `text [..N]`) | ENFORCED at field-load — out-of-range input `sys_exit(1)`s before the rule body runs (PR #35 for numbers, PR #146 for text; the text half was an argv-controlled stack overflow) | **not emitted.** gen0's trampolines load the field and run. Measured: `examples/aes_sbox.verbose` (`b : number [0, 255]`) given 256 / 300 / −5 returns a value at rc 0 where verbosec's binary exits 1. Pre-existing and independent of any parser slice — reproduced with an unpatched gen0 on `examples/even_odd.verbose`. Same "declaration exploited but unenforced" class as #146 |
| `byte_at(<text>, <i>)` bounds | fail-closed — `i >= length(text)` (and, via the unsigned compare, `i < 0`) `sys_exit(1)`s | **no check.** An out-of-bounds read returns 0 (or whatever the region holds) at rc 0. Measured with a minimal probe (`byte_at(h.data, 20)` over `"short"`): verbosec exits 1 with no output, gen0 prints `0` and exits 0. Pre-existing — gen0 built from unpatched `main` emits a **byte-identical** ELF for that probe (sha256 `4773295c…`), so no parser slice is implicated. This is what made `field_codec` / `aes_gctr` diverge in the 2026-08-06 differential sweep; both AGREE with verbosec on every in-bounds input. `substring` bounds are presumably in the same state and were not probed |
