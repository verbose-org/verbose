...
- **Shared-memory service state** — a `service` MAY declare `state:` fields that persist across requests (see the Language Features entry in `CLAUDE.md`), but they live in the process's own frame slots. Under `concurrency: forked` a child inherits them by COW and its mutations do NOT propagate back — **and that is a consequence of the refusal below, not a gap.** Making a counter global across forked children means shared memory, which is refused on principle. So the honest statement of the limitation is "state is per-process, and processes do not share memory", not "state propagation is unbuilt". A future design point would have to argue its way past the next bullet first.
