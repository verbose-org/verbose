...
| **`service listen`** | Top-level `service <name> { listen: { protocol, port, max_request }, handler: <rule>, ... }` | Handler input/output types match the protocol's built-in concepts (`HttpRequest`/`HttpResponse` for `http_1_0`, byte concepts for `raw_tcp`) | `socket` → `setsockopt(SO_REUSEADDR)` → `bind(:port)` → `listen(128)` → loop: `accept` → handler-emitted body → `close`. **Multi-step `raw_tcp`** (slice `multistep-1`, 2026-09-05): a `raw_tcp` service declaring `max_steps` + `read_timeout` (both mandatory together, `concurrency: forked` mandatory) runs, in the CHILD, `setsockopt(client_fd, SOL_SOCKET, SO_RCVTIMEO, {S, 0}, 16)` then a per-connection STEP LOOP: `cmp step, max_steps ; jae close` → `read` → `jle close` (EOF, error, and the timeout's `-EAGAIN` — no new branch) → handler → response streamed to the client fd → `after:` block → `lea rsp ; jmp step_top`; `close` then `sys_exit(0)`. One frame per read; a frame spanning two reads is two frames. | Per-iteration; if HTTP parse fails, close client_fd and loop. Server itself does not exit. | `max_request` u32 caps per-connection read buffer | Port and protocol in source; protocol is from a closed set (`raw_tcp`, `http_1_0`); no user-written wire parsers | Top-level `service` declaration |
| **`SO_RCVTIMEO` on the accepted socket** (slice `multistep-1`, 2026-09-05) | `service { protocol: raw_tcp, concurrency: forked, max_steps: N, read_timeout: S, ... }` — `1 ≤ N ≤ 65535`, `1 ≤ S ≤ 3600`, both mandatory together | None beyond the step-loop shape (the two knobs are declarations the emitter applies; the verifier refuses one without the other, either on `http_1_0`, and multi-step without `forked`) | In the child, after `fork`: `setsockopt(client_fd, SOL_SOCKET=1, SO_RCVTIMEO=20, &timeval{S, 0}, 16)`, once per connection. A `read` that waits longer than `S` returns `-EAGAIN`, which the step loop's existing `test rax,rax ; jle close` treats as close. The step cap is `cmp qword [rbp-56], N ; jae close` at the top of the loop, BEFORE the read, so the child closes the moment its `N`-th response is written | Timeout → close the connection, child exits 0, listener untouched. Step cap → same | The declared per-connection ceiling is `N × S` seconds of READ time and `N` frames of work; both factors are one `grep` away. **NOT bounded: a `write`** — a client that never reads can stall the child in the response write on a full socket buffer (`SO_SNDTIMEO` is a later slice, because a streamed response is many small writes and a partial write's meaning is a semantic question). "Nothing unbounded compiles" is NOT a claim this effect makes | `max_steps` / `read_timeout` / `concurrency: forked` are one keyword each in the source; a disassembler shows `mov rdx, 20` beside the `timeval` | `raw_tcp` services only |
| **`fork()` per accept** | `service { ..., concurrency: forked }` (default `sequential`) | None (orthogonal to handler logic) | At startup: `rt_sigaction(SIGCHLD, SIG_IGN, NULL, 8)`. Per accept: `fork()`. Parent: `close(client_fd) ; jmp accept_top`. Child: runs the iteration body, then `sys_exit(0)`. Fork failure: `write(2, "fork failed\n", 12) ; close(client_fd) ; jmp accept_top` (drop the connection, keep serving). | Fork failure → drop-and-continue (transient resource exhaustion shouldn't kill the server). Child failures → child exit only. | None (no per-fork buffer growth) | `concurrency: forked` line is in source; one keyword tells the auditor the binary spawns children | Service-level only; `Protocol::Http10`, or `Protocol::RawTcp` WITH a step loop (`max_steps` + `read_timeout`, slice `multistep-1` — where it is MANDATORY, §5.4 of `docs/multistep-connection-design.md`: a sequential server holding one connection across a whole conversation is monopolised for as long as the peer chooses). A one-shot `raw_tcp` service stays sequential. |
...
| **`random(<entropy>)`** — kernel CSPRNG bytes (slice `entropy-1`, 2026-09-02; design: `docs/randomness-effect-design.md`) | Top-level `entropy <name> { bytes: N, on_draw_error: abort }` (`on_draw_error` optional, `abort` the only value; no `cache:` field — refused by name, a cached draw would be inherited by every forked child) | Rule's `reads:` lists the entropy name, checked both ways like a resource name (`missing: [k]` / `extra: [k]`) | `getrandom(&buf, N, 0)` — `syscall 318` on x86-64 — **once per declared name per evaluation**: per input record in a rule binary (inside `loop_top`, so per line under `--stream`), per request in a service (after `fork`, after the HTTP parse). Flags word is the constant 0: no `GRND_RANDOM`, no `GRND_NONBLOCK`, no `GRND_INSECURE` — asserted on the emitted bytes, not trusted | `abort` only. `cmp rax, N ; jne abort` — a return value `≠ N` (a `-errno`, or a short read, unreachable for `N ≤ 256` per the syscall contract) → `sys_exit(1)` via the shared abort label. In a listener this kills the process (sequential) or the child (forked), and that is correct because no client can trigger it: with flags 0 and `N ≤ 256` the only reachable errno is `ENOSYS` (kernel < 3.17), permanent and process-level. Blocking before the pool is initialized is not a failure — the syscall waits, which is the desired semantics | `N` is a literal, verifier-bounded `1..=256` — the bound below which `getrandom(2)` guarantees a full, uninterrupted read on an initialized pool, so the emitter stores the DECLARED width as the length (a fact, not an assertion). Stack buffer of `N` bytes (padded to 8) + `(ptr, len)` = `16 + ((N + 7) & !7)` bytes per referenced name, in the resource / connection block of the frame | **Source**: the `entropy` block enumerates every draw and its width; the using rule's `reads:` names it. **Binary**: implicit, like `clock_gettime` — `strings` shows nothing (no path, no host, no port, no `/dev/*random`); a disassembler shows `mov rax, 318` | Rule logic and `let` RHS in every emitter built on `emit_record_loop_prologue` (scalar / bool / `Result` / record / `text` / `bytes` outputs); HTTP service handler logic and handler `let`s. Consumed by `byte_at`, `length`, and as the whole body of an `output: bytes` rule — the value is `bytes`, so every TEXT sink (`concat`, `HttpResponse.body`, the `log:` grammar, text `==`, `after:` sets) refuses it and exposing a draw is always an explicit, visible `byte_at`. **NOT** in `log:` content (never — an audit log must see that a draw happened, not its value), `after:` sets, reactions, `raw_tcp` handlers, collection / fold / parallel / vectorized rules, the callable path (recursion, record-returning or cross-concept callees), or as one argument of a bytes `concat` — each refused with a breadcrumb naming the lifting slice |

## Cross-cutting doctrine

### Naming convention

Every effect that touches the world has a **handle**: a top-level declaration with a name. `resource <name>`, `connection <name>`, `entropy <name>`, `service <name>`. The rule that uses it must reference the name in its purity proof's `reads:` list. The verifier checks the cross-reference both ways:

- Every name listed in `reads:` resolves to a declared field, resource, connection, or entropy item.
- Every `read(name)` / `fetch(name, _)` / `random(name)` resolves to a declared resource / connection / entropy item, and the three namespaces may not collide (they merge in `reads:`).
- A rule that references a resource via `read(...)` but does NOT list it in `reads:` is rejected with "declared reads do not match logic; missing: [...]" — and one that lists a name it never touches with "extra: [...]".
...
- `entropy { ..., on_draw_error: abort }` (slice entropy-1, the only option — and, unlike the two above, not "today": `drop` is refused by name, because entropy failure is process-level and never client-triggerable, so dropping would loop on every request hiding a deployment error)
...
- `random(<entropy>)` → NOTHING in `strings` (no path, no host, no device — `/dev/urandom` never appears, because the kernel interface is a syscall, not a file). Like `clock_gettime`, it is a disassembler-level marker: `mov rax, 318` immediately followed by a zero flags word (`xor edx, edx`) and `syscall`. The `entropy` block in the source is the audit surface; `entropy_flags_word_is_zero_and_no_userspace_source` is the mechanical check that the binary carries exactly that shape and nothing else.
...
- **`random(<entropy>)`** (slice entropy-1) is usable in the logic and `let` RHS of every rule built on the record-loop prologue (scalar / bool / `Result` / record / `text` / `bytes` outputs) and in HTTP handler logic and handler `let`s — consumed by `byte_at`, `length`, or as the whole body of an `output: bytes` rule. Refused, each with a breadcrumb naming the lifting slice: the callable path (entropy-4), collection / fold / multi-fold / parallel / vectorized rules (entropy-5), reactions and `after:` sets (entropy-6), `raw_tcp` handlers and a draw as one argument of a bytes `concat` (entropy-2). `log:` content: never.
...
- ~~**POST/PUT request bodies**~~ — **SHIPPED 2026-04-29, this entry was stale.** `req.body` is parsed (the parser scans for `\r\n\r\n` after method/path and stores the body's `(ptr, len)` at dedicated rbp slots) and is available in handler logic and in `log:` content, composing as BoundText through `length` / `starts_with` / `contains` / `concat` / `json_escape`. Worked examples: `examples/body_size_gate.verbose` (1357 B — returns 413 when `length(req.body) > parse_int(read(max_body))`) and `examples/body_content_gate.verbose` (2678 B). Listing a shipped feature as refused is the "refused vs unbuilt" confusion in its worst direction: this file is the closed effect catalogue, so a stale refusal here is load-bearing misinformation, not a typo.
...
- **Shared-memory service state** — a `service` MAY declare `state:` fields that persist across requests (see the Language Features entry in `CLAUDE.md`), but they live in the process's own frame slots, so **an `after:` mutation together with `concurrency: forked` is REFUSED at verify time** (2026-08-30). This entry previously said only that a child's mutations "do NOT propagate back", which reads as degraded-but-useful and is not what happens: `fork()` is per-`accept` and http_1_0 serves one request per connection, so every child starts from the parent's *unchanged* slots, mutates its own COW page, and exits — the mutation is observed by **nobody, ever**, and the state reads as a constant. Measured: the counter service answering `count:0 / 1 / 2 / 3` sequentially answers `count:0` to every request once `concurrency: forked` is added. Making it propagate means shared memory, which the next bullet refuses on principle, so the combination is **unbuildable, not unbuilt** — and a declaration that means nothing is the *false explicitation* `CLAUDE.md` forbids by name, which is why the verifier now refuses the pair instead of documenting it. The refusal is keyed on `after_sets`, not `state_fields`: a `state:` block with no `after:` block is a per-process constant, identical under both modes, and refusing it would reject a valid program. **RE-KEYED by slice `multistep-1` (2026-09-05) to `after_sets && forked && no step loop`**: a `raw_tcp` service with a step loop (`max_steps` + `read_timeout`) runs one child per CONNECTION, and inside that child the `after:` block runs at the bottom of the step loop, so the mutation IS observed — by the next step of the same connection. `state:` then has two lifetimes among accepted programs, per process (sequential `http_1_0`) and per connection (forked multi-step `raw_tcp`), under one rule: *`after:` runs at the bottom of the innermost loop the service declares, and a `state:` field lives for the frame that encloses that loop* (design §6.1). Per-connection reset is the fork itself — `STATE INIT` runs once in the parent before `listen`, every child inherits the initialised slots by copy-on-write — zero instructions. `state:` on `raw_tcp` WITHOUT a step loop is refused (the one-shot echo emitter is handed two scalars and would drop the block silently); `bytes` state is refused pending `state-read-typing` (design §6.2). A future design point would still have to argue its way past the next bullet first.
