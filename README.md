...
- **Typed results.** `Result(T, E)` makes the failure path a declared part of the output instead of an implicit panic. `Ok(value)` and `Err(reason)` are the two arms, both visible to the caller and audited at the IR level. `match_result(r, v => ok_body, e => err_body)` consumes a Result — both arms named and required, no implicit Err-propagation, so the reader sees exactly what happens on failure. Already implemented — a rule can declare `output: Result(number, text)`, return either form, and compose with other Result-returning rules.
