...
| `deadcode.verbose` | Native elimination of a proved unreachable branch (`number [0, 50]`), after checking both branches and retaining the input guard. |
...
- [`capped_counter.verbose`](capped_counter.verbose): a comparison with a configured ceiling proves an increment safe, including at i64::MAX. Scalar comparisons transfer checked bounds within their branch, with no runtime proof storage.
- [`guarded_total.verbose`](guarded_total.verbose): explicit zero/positive quantity branches whose redundant test and zero multiplication are removed only after verifying the complete source. Original numeric input guards remain.
- [`guarded_magnitude.verbose`](guarded_magnitude.verbose): a nonzero signed quantity has positive absolute magnitude. Native simplification retains both sign intervals and removes the redundant magnitude test after verifying all branches.
- [`scalar_identities.verbose`](scalar_identities.verbose): guarded squaring over signed input, nonzero self-division and exact cancellation. Verification recognizes two reads of the same scalar, checks every original operation, and native lowering precomputes constant results.
