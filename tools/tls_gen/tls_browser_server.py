"""Certificate-based TLS 1.3 server hardened for real browsers: HelloRetryRequest
(HRR), multi-record ClientHello reassembly, ALPN (http/1.1), a fork-per-connection
accept loop, and a real HTTP/1.1 HTML response.
...
untouched as the known-good baseline):
  ...
  2. HelloRetryRequest — if the client did NOT send an X25519 key_share but
     ...
     This is exactly the Chrome case where CH1's only key_share is for an
     unsupported group (e.g. X25519MLKEM768 0x11ec): the MLKEM KeyShareEntry is
     skipped by the parser, x25519_pub is None, and HRR fires. If Chrome sends
     BOTH an MLKEM and an X25519 key_share in CH1, x25519_pub is populated and we
     take the direct path, ignoring the MLKEM entry. Firefox leads with X25519
     (direct path).

  3. ALPN (RFC 7301) — if the client offers ALPN, select "http/1.1" (we have no
     HTTP/2 framing) and echo it back in EncryptedExtensions. Omitted if the
     client did not offer ALPN.

  4. Fork-per-connection accept loop — a browser opens several parallel TLS
     connections and may abandon some mid-handshake. The parent loops accept()ing;
     each accepted connection is handled in a forked child that exits when done.
     SIGCHLD=SIG_IGN auto-reaps children (no zombies). A crash/hang/abort in one
     child never touches the parent or sibling connections.

  5. Real HTTP/1.1 response — the server reads one decrypted app-data record (the
     browser's GET request) then serves a small HTML page with Content-Length and
     Connection: close so the browser renders and the connection ends cleanly.
...
HTML_BODY = (b"<!doctype html><meta charset=utf-8>"
             ...
             b"(X25519, AES-128-GCM, SHA-256, Ed25519) runs on Verbose-emitted "
             b"machine code.</p>")
...
def serve(conn):
    ...
    log(f"CH1: x25519_keyshare={'yes' if ch1.x25519_pub else 'no'} "
        ...
        f"offers_ed25519={0x0807 in getattr(ch1,'sig_algs',[])}")
    ...
    # The browser/openssl sends `GET / HTTP/1.1\r\nHost: ...` shortly after its
    # Finished. We don't route on it (one page for every path) but we MUST read
    # it so the response is correctly ordered after the request. Tolerate the
    # request not having arrived yet by waiting briefly on the socket.
    req_summary = "none"
    try:
        conn.settimeout(10.0)
        rec, buf = read_record(buf)
        # A client may interleave a post-handshake CCS or a session-ticket-less
        # NewSessionTicket is server-sent only; here skip any non-app records.
...
def main():
    ...
    # SIG_IGN on SIGCHLD makes the kernel discard child exit status (Linux),
    # which is exactly what we want — we never waitpid() on a specific child.
    ...
    while True:
        try:
            ...
            # accept() interrupted by SIGCHLD delivery — just retry.
            continue
        except OSError as e:
            print(f"accept error: {e}", flush=True)
            continue

        # Fork per connection: the child owns the handshake end-to-end and exits;
        # the parent immediately closes its copy of the client fd and goes back
        # to accept(). A browser opens several parallel connections (favicon,
        # preconnect, keep-alive probes) and may abandon some mid-handshake; a
        # crash or hang in one child can never take down the accept loop.
        try:
            pid = os.fork()
        except OSError as e:
        ...
        if pid == 0:
            ...
            try:
                ...
                rc = 1
            except Exception as e:           # never let an unexpected error
                print(f"unexpected child error: {e!r}", flush=True)
                rc = 1
            finally:
                try: conn.close()
                except OSError: pass
            os._exit(rc)                     # child exits; never returns to loop
        else:
            # --- parent ---
            conn.close()                     # parent drops its copy of client fd
            # No waitpid here: SIGCHLD=SIG_IGN auto-reaps. Loop and keep serving.
